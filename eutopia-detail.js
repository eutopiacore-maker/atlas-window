...
export function buildEutopia(scene,PX,PZ){
 ...
 const ground=new THREE.Mesh(new THREE.PlaneGeometry(150,120),mat(0x496f43,.98));ground.rotation.x=-Math.PI/2;ground.position.set(PX,1.68,PZ);ground.receiveShadow=true;g.add(ground);
 for(let i=0;i<34;i++){const a=i*.83,r=32+(i%7)*8,h=3+(i%5)*2;const m=new THREE.Mesh(new THREE.ConeGeometry(10+(i%4)*4,h,16),mat(i%3?0x315f3b:0x4d7548));m.position.set(PX+Math.cos(a)*r,h/2+1.3,PZ+Math.sin(a)*r);m.scale.y=.55;g.add(m)}
 const park=new THREE.Mesh(new THREE.PlaneGeometry(55,13),mat(0x5d864b));park.rotation.x=-Math.PI/2;park.position.set(PX,1.76,PZ);g.add(park);
