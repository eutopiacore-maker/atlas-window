...
async function publish(){const s=status();atomic(path.join(STATE,'node-status.json'),JSON.stringify(s,null,2));if(TOKEN&&Date.now()-lastPublish>15*60*1000){try{await putRepoText(`pc-node/nodes/${ID}.json`,JSON.stringify(s,null,2)+'\n',`Atlas node status ${ID}`);lastPublish=Date.now()}catch(e){log('status publish failed',e.message)}}}
