...
const PORT=8765;
const THREE_URL='https://cdn.jsdelivr.net/npm/three@0.168.0/build/three.module.js';
const REMOTE_HEARTBEAT_MS=6*60*60*1000;
for(const d of[STATE,LOGS,WEB,WORLD,ADDONS,path.join(WEB,'vendor')])fs.mkdirSync(d,{recursive:true});
...
function readJson(f,d=null){try{return JSON.parse(fs.readFileSync(f,'utf8').replace(/^\uFEFF/,''))}catch{return d}}
...
const ID=nodeId();
let desired=null,catalog=null,online=false,lastPull=0,lastWeb=0,restart=false;
...
function installedAddons(){return readJson(path.join(ADDONS,'installed.json'),{addons:{}}).addons||{}}
function mergedCatalog(){const installed=installedAddons();return{schema:catalog?.schema||1,updatedAt:catalog?.updatedAt||null,addons:(catalog?.addons||[]).filter(a=>!a.internalOnly).map(a=>{const i=installed[a.id];return{...a,status:i?(i.version===a.version?'installed':'update'):a.status}})}}
...
function remoteStatus(){const s=status(),w=s.world||{},inv=s.inventory||{},adds=installedAddons();return{schema:1,nodeId:ID,state:s.state,online:s.online,runtimeVersion:s.runtimeVersion,hardware:{platform:inv.platform,arch:inv.arch,ramBytes:inv.ramBytes,gpus:(inv.gpus||[]).map(g=>({name:g.Name||g.name||null,adapterRAM:g.AdapterRAM??g.adapterRAM??null,driverVersion:g.DriverVersion||g.driverVersion||null}))},world:{state:w.state||null,package:w.package||null},addons:Object.fromEntries(Object.entries(adds).map(([id,v])=>[id,{version:v.version,healthy:!!v.healthy}])),desiredGeneration:s.desiredGeneration}}
...
const WEB_FILES=['index.html','world.html','dynamics.html','dynamics.js','appearance.html','appearance-scene.js','appearance-decoder.json','eutopia-detail.js','host.html'];
function patchOfflineImports(){for(const f of['dynamics.js','dynamics.html']){const p=path.join(WEB,f);if(fs.existsSync(p)){const s=fs.readFileSync(p,'utf8').split(THREE_URL).join('./vendor/three.module.js');fs.writeFileSync(p,s)}}}
async function syncWeb(){if(!online||Date.now()-lastWeb<300000)return;lastWeb=Date.now();for(const f of WEB_FILES){try{atomic(path.join(WEB,f),(await repoText(f)).text)}catch(e){log('web sync',f,e.message)}}try{const r=await request(THREE_URL,{headers:{Accept:'text/javascript'}});if(r.status>=200&&r.status<300)fs.writeFileSync(path.join(WEB,'vendor','three.module.js'),r.body)}catch(e){log('three cache',e.message)}patchOfflineImports()}
async function publish(){const local=status();atomic(path.join(STATE,'node-status.json'),JSON.stringify(local,null,2));if(!TOKEN)return;const pub=remoteStatus(),stable=JSON.stringify(pub),sig=crypto.createHash('sha256').update(stable).digest('hex'),metaFile=path.join(STATE,'remote-status-meta.json'),meta=readJson(metaFile,{}),due=Date.now()-Date.parse(meta.at||0)>REMOTE_HEARTBEAT_MS;if(sig===meta.sig&&!due)return;try{const payload={...pub,reportedAt:new Date().toISOString()};await putRepoText(`pc-node/nodes/${ID}.json`,JSON.stringify(payload,null,2)+'\n',`Atlas node status ${ID}`);atomic(metaFile,JSON.stringify({sig,at:payload.reportedAt},null,2))}catch(e){log('status publish failed',e.message)}}
